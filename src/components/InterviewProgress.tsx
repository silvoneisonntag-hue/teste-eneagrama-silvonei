...
const PHASES = [
  { key: "acolhimento", label: "Acolhimento", short: "Acolh." },
  { key: "emocional", label: "Emocional", short: "Emoc." },
  { key: "comportamental", label: "Comportamental", short: "Comport." },
  { key: "sistemico", label: "Sistêmico", short: "Sistêm." },
...
const InterviewProgress = ({ currentPhase }: InterviewProgressProps) => {
  ...
  return (
    <div className="px-3 sm:px-4 py-2.5 border-b border-border/50 bg-card/50 backdrop-blur-sm">
      {/* Progress bar track */}
            ...
            <div
              ...
              className={`absolute top-1/2 -translate-y-1/2 w-2.5 h-2.5 rounded-full border-2 transition-all ${
                isCompleted
                  ...
                  : isCurrent
                  ? "bg-accent/30 border-accent shadow-[0_0_6px_hsl(var(--accent)/0.5)]"
                  : "bg-muted border-border"
              }`}
              style={{ left: `${left}%`, transform: "translate(-50%, -50%)" }}
            >
              {isCompleted && <Check className="w-1.5 h-1.5 text-accent-foreground absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" />}
            </div>
          );
        })}
      </div>
